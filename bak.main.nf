...
def is_large_genome = {hisat2_index_dir ->
    index_files = file("${hisat2_index_dir}/*.ht2l")
...
process hisat2_sortOutput {
    ...
    file "${name}.sorted.bam.*i" into bam_for_genebody_idx
    ...
    if (is_large_genome(params.hisat2_index)) {
        index_params = "-c"
    } else {
        index_params = ""
    }
    ...
    samtools index ${index_params} ${name}.sorted.bam
    """
}
...
process rseqc {
    ...
    when:
    !params.skip_qc && !params.skip_rseqc && !is_large_genome(params.hisat2_index)
...
process markDuplicates {
    ...
    samtools index ${index_params} ${name}.markDups.bam
    """
}
