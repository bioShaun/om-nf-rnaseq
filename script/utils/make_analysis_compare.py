...
def rmats_sample_files(bam_dir, sample_group, out_dir, contrast=None):
    ...
    sample_df = pd.read_csv(sample_group,
                            header=None,
                            names=['sample_id'],
                            index_col=1,
                            sep='\t')
    ...
    if contrast is not None:
        ...
        contrast_obj = itertools.combinations(sample_df.index.unique(), 2)
    ...
    for each_comp in contrast_obj:
        ...
        for n, comp in enumerate(each_comp):
            ...
            bam_files = [
                str(bam_dir / f'{sample}.sorted.bam')
...
if __name__ == '__main__':
    fire.Fire({
        'rmats_sample_files': rmats_sample_files,
