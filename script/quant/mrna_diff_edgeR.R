...
om_lnc_volcano_plot <- function(dpa_results, compare_name,
  ...
  diff_count <- table(dpa_results$status)
  diff_label <- paste(names(diff_count), diff_count, sep=': ')
  names(diff_label) <- names(diff_count)
  dpa_results$diff_num <- diff_count[dpa_results$status]
  dpa_results$plot_label <- paste(dpa_results$status, dpa_results$diff_num, sep=': ')
  ...
  dpa_results$status <- factor(dpa_results$status,
                                     levels = plot_order)
  p <- ggplot(dpa_results, aes(logFC, logFDR, colour = status)) +
    geom_point(size=0.6) +
