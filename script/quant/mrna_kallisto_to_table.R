...
om_lnc_boxplot <- function(exp_data, samples, 
                           outdir=NULL) {
  ...
  if (! is.null(outdir)) {
    ...
    plot_width = 3 + sample_num/6
    plot_height = 6 + sample_num/6
...
om_lnc_boxplot(gene_tpm_matrix, samples,
           outdir=expression_stat_dir)
om_pca_plot(gene_tpm_matrix, samples, outdir = expression_stat_dir)
om_lnc_correlation_plot(gene_tpm_matrix, samples, outdir = expression_stat_dir)
