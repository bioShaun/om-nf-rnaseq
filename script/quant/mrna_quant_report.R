...
argv <- parse_args(p)
	...
	gene_type_heatmap <- function (exp_df, samples, out_dir) {
	  ...
	  if (diff_gene_count <= DIFF_HEATMAP_GENE) {
	    om_lnc_heatmap(diff_gene_tpm_matrix, samples=samples, outdir=out_dir)
	  } else {
	    ...
	    om_lnc_heatmap(diff_gene_tpm_matrix, samples, outdir=out_dir)
	    diff_gene_count <- dim(diff_gene_tpm_matrix)[1]
	  }
	...
	om_lnc_cluster_plot <- function(plot_data, out_prefix=NULL, title="") {
	  ...
	  cluster_plot <- ggplot(plot_data, aes(x=variable, y=value, group = Gene_ID, color="#8DA0CB")) +
	    geom_line(alpha = 0.5) +
	...
	if (dim(out_diff_gene_raw_count_matrix)[1] > 0) {
	  ...
	  gene_type_heatmap(out_diff_gene_tpm_matrix, samples, out_dir)
